...
export default function MessageBubble({ message }) {
  ...
  return (
    <div className={cn("flex gap-3", isUser ? "justify-end" : "justify-start")}>
      {!isUser && (
        <div className="h-7 w-7 rounded-lg bg-violet-100 flex items-center justify-center mt-0.5 shrink-0">
      ...
      <div className={cn("max-w-[85%]", isUser && "flex flex-col items-end")}>
        {message.content && (
          <div className={cn("rounded-2xl px-4 py-2.5", isUser ? "bg-slate-800 text-white" : "bg-white border border-slate-200")}>
            {isUser ? (
              <p className="text-sm leading-relaxed" style={{ direction: 'rtl' }}>{message.content}</p>
            ) : (
              <ReactMarkdown
